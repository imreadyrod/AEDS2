...
public class q1 {
    public static void main(String[] args) throws Exception {
        ...
        while (!input.equals("FIM")){
            ...
            l.inserirInicio(personagem);
            input = MyIO.readLine().replaceAll("é", "\u00e9");
        }

        int repeticoes = Integer.parseInt(MyIO.readLine().replaceAll("é", "\u00e9"));
        input = MyIO.readLine().replaceAll("é", "\u00e9");
        while (repeticoes!=0){
            String[] comando = input.split(" ");
            
            if(comando.length==2 || comando.length==3){
                Personagem personagem = new Personagem();
                personagem.ler(comando[1]);
                if(comando[0]=="II"){
                    l.inserirInicio(personagem);
                }
                else if(comando[0] == "IF"){
                    l.inserirFim(personagem);
                }
                else if(comando[0]=="I*"){
                    int posicao = Integer.parseInt(comando[1]);
                    l.inserir(personagem, posicao);
                }
                else if(comando[0]=="R*"){
                    int posicao = Integer.parseInt(comando[1]);
                    l.remover(posicao).imprimir();
                }
            }
            if(input == "RI"){
                l.removerInicio().imprimir();
            }
            else if(input == "RF"){
...
class lista {
    ...
    public Personagem removerFim() throws Exception{
        ...
        celula i;
        for(i=primeiro;i.prox!=ultimo.prox;i=i.prox);
