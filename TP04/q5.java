...
public class q5 {
    public static void main(String[] args) throws Exception {
        ...
        arquivo.criarArquivoLog(matricula,tempoExecucao,h.numeroComparacoes);
    }
}
...
class hash{
    ...
    private final int tamanhoReserva = 9;
    int reserva;
    ...
    public int posicao(int altura){
        return altura%tamanho;
    }

    public void inserir(Personagem objPersonagem) {

        int hash = posicao(objPersonagem.getAltura());
        int indice = hash % (tamanho-tamanhoReserva);
        ...
        if (tabela[indice]== null) {
            ...
            while(tabela[reserva+(tamanho-tamanhoReserva)]!=null)
                incrementaComparacoes();             
                reserva++;
            }
            // Área de reserva, encontra o próximo espaço vazio e insere o personagem
            int indiceReserva = reserva+tamanhoReserva;
            if (indiceReserva != -1) {
                incrementaComparacoes();
                tabela[indiceReserva] = objPersonagem;
            } else {
                System.out.println("Tabela cheia, não é possível inserir: " + objPersonagem.getNome());
            }
    }
